...
def test_complete_dataframe(spark):
    ...
    actual = [(x.name, x.dataType) for x in complete_dataframe(mockInput, mockSchema).schema.fields]
...
def test_select_schema_cols(spark):
    ...
    actual = [(x.name) for x in select_schema_columns(mockInput, target_schema).schema.fields]
